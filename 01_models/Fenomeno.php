<?php
/* Esta clase va a gestionar la tabla `tiposfenomenos` */
...
class Fenomeno
{
  ...
  {
    $consultaSQL = "INSERT INTO fenomenos (nombre, descripcion) VALUES 
    ...
    */
    $consultaPrepare->bind_param("ss", $nombre, $descripcion); // Las fechas al ser de tipo TIMESTAMP en la DB, hay que ponerlas de tipo entero `i`
  ...
  {
    $id = intval($_GET['id']);
    $consultaSQL = "SELECT * FROM tiposfenomenos WHERE id = $id;"; // Aquí saco la info a través de su ID y sus Foreign Keys asociados
