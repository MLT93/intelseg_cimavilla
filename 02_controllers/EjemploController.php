...
require_once("01_models/Ejemplo.php");
...
class PruebaController
{
  ...
  public function index() {
    require "03_views/ejemplo/home.php";
  }

  public function list()
  {
    /* AQUÍ OBTENGO LA INFO PRINCIPAL DE LOS DE MI DB */
    $model = new Ejemplo(); // Instancia donde se realiza la conexión a la DB
    $lists = $model->getAll(); // Obtengo la información de la DB y la envío a la View `list.ph

    require "03_views/ejemplo/list.php";
  }
  ...
  public function register() // Nos manda al formulario de registro
  {
    ...
    require "03_views/ejemplo/form_register.php"; // Aquí estará disponible la variable sin utilizar
  }
  ...
  {
    $model = new Ejemplo(); // Instancia donde se realiza la conexión a la DB
    $details = $model->getByIDQueryParams(); // Obtengo la información de la DB y la envío a la View `detail.php`

    require "03_views/ejemplo/detail.php"; // Aquí estará disponible la variable sin utilizar
  }

  public function pathVariables($id) // Muestra a través del ID
  {
    $model = new Ejemplo(); // Instancia donde se realiza la conexión a la DB
    $details = $model->getByIDPathVariables($id); // Obtengo la información de la DB y la envío a la View `detail.php`
